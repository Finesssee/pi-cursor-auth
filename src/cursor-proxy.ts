 ...
 * HTTP/2 transport is delegated to a Node child process (cursor-h2-bridge.mjs).
 */
  ...
  const server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
    ...
    if (req.method === "POST" && url.pathname === "/v1/chat/completions") {
      try {
        ...
        handleChatCompletionNode(body, accessToken, res);
      } catch (err) {
...
// --- Chat Completion Handler (Node.js) ---

function handleChatCompletionNode(
  ...
  if (activeBridge && toolResults.length > 0) {
    ...
    handleToolResultResumeNode(activeBridge, toolResults, modelId, tools, accessToken, bridgeKey, res);
    return;
  }
  ...
  if (body.stream === false) {
    handleNonStreamingResponseNode(payload, accessToken, modelId, res);
    return;
  }
  handleStreamingResponseNode(payload, accessToken, modelId, bridgeKey, res);
}
...
function handleStreamingResponseNode(
  payload: CursorRequestPayload,
  accessToken: string,
  modelId: string,
  bridgeKey: string,
...
// --- Tool Result Resume (Node.js) ---

function handleToolResultResumeNode(
  active: ActiveBridge,
  toolResults: ToolResultInfo[],
  modelId: string,
  tools: OpenAIToolDef[],
  ...
  streamSSE(res, { requestBytes: new Uint8Array(0), blobStore, mcpTools }, accessToken, modelId, bridgeKey, bridge, heartbeatTimer, blobStore, mcpTools);
}

// --- Non-Streaming Handler (Node.js) ---

function handleNonStreamingResponseNode(
  payload: CursorRequestPayload,
  accessToken: string,
  modelId: string,
  res: ServerResponse,
